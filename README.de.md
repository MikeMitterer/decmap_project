...
- [x] Lokalisierte Tag-/Cluster-Namen (`tags.name_translations`, Migration 011): KI-generierte L1–L9-Cluster-Labels sind englisch und wurden bei **jedem** deutschen Listen-/Graph-Load live übersetzt (~13 `/ai/translate`-Calls je DE-Table-Load → nginx 5r/m 429 — der Haupttreiber des anzeige-seitigen Rate-Limits). Sie werden jetzt beim Clustering-Upsert in eine `name_translations`-JSONB-Spalte vorab-übersetzt (analog `problems.original_translations`; ON-CONFLICT-Refresh, kein Ignore), plus ein Einmal-`POST /clustering/backfill-tag-translations` für vor der Migration angelegte Tags (nur strukturelle L1–L9 ohne Übersetzung); das Frontend liest `nameTranslations[locale] ?? name` (`utils/tagDisplay.ts`) statt live zu übersetzen und schließt so das DE-Listen-429 (Backend `a355520` + ai-service `db6f41c` auf master; Frontend-Display-Umstellung `c9820da`/`580dae7` auf `feature/bold-redesign`)
- [x] Moderations-Fail-Safe bei Provider-Fehlern (2026-07-25): Ein fehlgeschlagener LLM-Spam-Filter-Aufruf (Rate-Limit, Quota-Erschöpfung, API-Ausfall) eskaliert ein Problem jetzt zu `needs_review` (`moderation_error`), statt es still abzulehnen — vorher lehnte eine Provider-Quota-Störung im Background-Hook jedes eingehende Problem ab. Auto-Reject bleibt echten Spam-Verdikten, Honeypot und ≥2 Signalen vorbehalten; symmetrisch zum Lösungs-Pfad (Kriterien: [`docs/moderation-criteria.md`](docs/moderation-criteria.md))

**Nächste Schritte:**
- [ ] Bold UI Redesign — Glass-Morphism mit Mesh-Orb-Ambient-Background (sichtbar durch transparente TopBar/StatusBar sowie Login/Status/Admin-Seiten), DmSidebar (72px) + DmTopBar App-Shell mit fixierter englischer Brand-Tagline (`MAP PROBLEMS · FIND SOLUTIONS`, nie übersetzt), Glass-Cytoscape-Graph mit Vote-Score-Badges, inset Cluster-Count-Badges, Cluster-Nodes die sich per Auto-Höhe an lange KI-generierte Labels anpassen (Problem-Nodes bleiben fixhoch, ihre Labels sind vorab getruncated), Cluster-Dot-Indikator pro Cluster eigene Farbe (deterministische 12-Farben-Palette via `utils/tagColor.ts`, geteilt mit dem Tabellen-Chip — jedes Cluster behält dieselbe Identitätsfarbe über alle Views), Brand-Gradient-Root-Node (Orange→Pink→Magenta→Violett) und Brand-Gradient-Edges Root→L1 (Orange→Magenta), Radial-Layout mit Geschwister-Dichte-Kollisionsvermeidung (`r ≥ nodeSpacing × N / span`), Mindmap öffnet bei 100% Zoom mit Root-Node zentriert (`centerOnRoot()` stellt — falls vorhanden — den gespeicherten Viewport wieder her, sonst 100% Zoom auf Root, kein `fit` beim ersten Layout), ziehbare Nodes mit Decoration-Badges die bei jeder Positionsänderung mitwandern (`cy.on('position', selector, repositionBadgesFor)`) und User-gezogene Positionen in localStorage persistiert (`graph-node-positions`, beim nächsten Layout via `applyStoredPositions()` wiederhergestellt — algorithmische Position nur für Nodes ohne gespeicherte Position; Positionen sind pro View-Ebene gescoped (Mindmap vs. gedrillter Cluster), sodass ein im Überblick verschobener Node auf jeder Drill-Ebene seine eigene Position behält) sowie der Mindmap-Viewport (Zoom + Pan) mit 400ms-Debounce in `graph-viewport-mindmap` auto-gespeichert (`cy.on('viewport', ...)` nur bei `viewLevel === 'mindmap'`, Reload stellt den exakten Zustand wieder her) mit Reset-Positions-Button neben den Zoom-Controls (löscht beide Storage-Keys und re-runt das aktive Layout — `showMindmap` / `showUnclusteredView` / `applyTagFilters` — sodass wieder die algorithmischen Positionen und der Default-Viewport wirken), Glass-Tabellenansicht (8px-Rounded-Header, Cluster-Chip nur mit Punkt in Cluster-Farbe — Text und Hintergrund bleiben neutral, damit die Zeile ruhig liest; der Punkt allein trägt die Identität), vereinfachte Settings (Mode + 6 Accent-Swatches + ein separater KI-Akzent-Picker (6 Swatches, leer = Theme-Default, persistiert via `dm-ai-accent`, über Theme-Wechsel hinweg wiederhergestellt) + Gradient-Slider, persistiert via `dm-accent` / `dm-grad-strength`; in zwei stets sichtbare Tabs gegliedert — Profil (login-gated: Gäste sehen einen Login-Hinweis statt des Formulars) und Erscheinungsbild (ohne Login zugänglich)), Dark-Glass-Out-of-Box-Default beim Erstbesuch (dunkles Glass-Theme + Sunset-Accent + Fuchsia-KI-Akzent; OS-Hell/Dunkel-Following bleibt opt-in über eine explizite „system"-Preference), Live-Systemstatus-Label und Live-Graph-Zoom-Anzeige im Footer (`useGraphZoom`, sichtbar nur auf dem Graph-View; der Prozentwert selbst in der Accent-Farbe, das Wort „Zoom" faint), die neben der Zoom-Anzeige auch die Maus-/Tastatur-Navigation des Graphen als Inline-Hinweis zeigt (Scroll = Zoom, ⇧+Scroll = horizontal und Ziehen = verschieben inline, mit den Aktionswörtern in der Accent-Farbe; die volle Liste — ⇧+Strg+Scroll = vertikal, Pfeiltasten = verschieben/zwischen Problem-Nodes navigieren — im `title`-Tooltip; unter `md` ausgeblendet), auth-sensitiver Lösungs-CTA (Full-Width-Accent-Button in einem Sticky-Action-Footer am Panel-Boden — neben Vote-Pill (mit „eine Stimme pro Problem"-Tooltip, sobald abgestimmt) und Lösungs-Zähler (T-16) —, sodass er sichtbar bleibt, während die Liste darunter scrollt (`SolutionList` ist jetzt reine Liste); Plus-Icon + „Lösung hinzufügen" für eingeloggte User, Lock-Icon + „Anmelden, um beizutragen" für Gäste; beide Varianten rufen dasselbe lokale `showSolutionForm()` (kein `add`-Emit mehr, da der CTA jetzt in `ProblemPanel` statt `SolutionList` sitzt), das für Gäste View + Problem + Intent kodiert (`?problem=<id>&solution=new`), sodass der User nach dem Login in derselben View landet — mit wiederhergestelltem Panel und erneut geöffnetem Lösungs-Formular), neu gestaltete Lösungs-Karten (Header-Zeile mit Accent-farbiger Avatar-Pille + Author-Name in Space Grotesk + geteiltes `AiBadge` (Sparkle-Icon im eigenen `--th-ai-accent`, bewusst distinkt vom User-CTA-Akzent) — ersetzt das 🤖-Emoji-Prefix und vereinheitlicht sechs zuvor divergente `is_ai_generated`-Badges, während das KI-Marker-Designsystem über alle KI-Touchpoints ausgerollt wird ([decmap_project#39](https://github.com/MikeMitterer/decmap_project/issues/39)) — + faint mono Vote-Score; Content als 2-Zeilen-Preview via `stripMarkdown()` + Tailwind `line-clamp-2` statt harter Zeichen-Limit, sodass die Trunkierung mit der Panel-Breite skaliert — identisches Karten-Vokabular im Graph-Lösungs-Popup, damit Right-Panel und Popup eine gemeinsame visuelle Sprache teilen), Brand-Gradient ausschließlich für Identitäts-Flächen (Logo-Wordmark, Root-Node, Cluster→Root-Edges, Login-Headline — alle anderen CTAs nutzen die User-gewählte Akzentfarbe), EN/DE-Pill-Switcher absolut positioniert oben rechts auf `/login` (gleiche `LanguageSwitcher`-Component wie in der App-Shell, vor dem Anmelden sichtbar damit Erstbesucher die Sprache wechseln können ohne sich einzuloggen), DSGVO-konforme Self-Hosted Fonts, Neues-Problem-Formular öffnet als Centered Modal (T-12 — `<Teleport to="body">` aus `ProblemForm.vue` selbst, 680px R-02 Glass-Surface, Sticky-Footer mit `form="problem-form"`-Submit-Attribut, Similarity-Card inline zwischen Titel und Beschreibung; Problem-Detail bleibt im Side-Panel), Lösungs-Formular öffnet als Split-Editor-Modal (T-13 — self-contained `<Teleport to="body">` aus `SolutionForm.vue` selbst, 1080px R-02 Glass-Surface, permanenter Write | Live-Preview-Split statt der früheren Schreiben/Vorschau-Tabs, funktionale Markdown-Toolbar (Fett/Kursiv/Überschrift/Zitat/Liste/Link — umschließt die Auswahl oder setzt ein Zeilen-Präfix am Cursor), ESC/Backdrop-Schließen, ⌘+Enter zum Absenden; das Side-Panel zeigt weiter das Problem-Detail hinter dem Backdrop) sowie eine read-only Lösungs-Vollansicht (T-11 — neue Client-only-Route `/problem/:problemId/solution/:solutionId`, bewusst `ssr: false, prerender: false`, da die View Lösungen zur Laufzeit lädt (`fetchSolutions(problemId)` + `.find(id)`, kein by-id-Endpoint) und Autor-Namen clientseitig auflöst — anders als die prerenderten Problem-/Cluster-Seiten also **keine** SEO-Fläche; Zwei-Spalten-Reading-Layout — Markdown-Reading-Column neben einer Rail mit den übrigen Lösungen des Problems, die per Klick die aktive wechselt, kein erfundener Titel und kein Job-Titel, fremde Autoren als „Anonymous" und KI-Einträge über das geteilte `AiBadge`, Übersetzung folgt dem globalen EN/DE-Display-Locale (kein Per-Item-Toggle), Voting patcht sowohl den lokalen Score als auch den geteilten `solutions.value`-Eintrag, sodass der Score die Rail-Navigation überlebt, Share-Permalink und kein Bookmark; Back / „← Mindmap" stellt den kompletten Graph-View wieder her — Selektion + Drill-Level + Zoom/Pan, gespiegelt über `?problem=`/`?cluster=` und im `?from=` mitgeführt (die Rail-Links reichen denselben Origin weiter, damit ein Lösungs-Wechsel im Rail das Back-Ziel behält, während der Share-Permalink origin-frei bleibt), das selektierte Node nur mit Akzent-Rahmen statt Overlay-Corona; geöffnet über einen „↗ In voller Ansicht öffnen"-Link auf den Lösungs-Karten, mit `stripMarkdown` + Autor-Namen/Initial-Helfern nach `utils/solutionDisplay.ts` extrahiert und mit der Karten-Liste geteilt, dazu ein sticky, einklappbares Problem-Kontext-Band (T-14) innerhalb der Reading-Column (fluchtet mit der 680px-Lesespalte, sodass Auf-/Zuklappen nur den Solution-Content darunter bewegt und der rechte Rail unberührt bleibt) — default eingeklappt auf eine kompakte einzeilige Headline (Eyebrow + Titel + beschriftete Vote-Pill + ▲/▼-Toggle; die Headline-Zeile selbst ist ebenfalls ein Klick-/Tastatur-Toggle, gemeinsamer `isExpandable`-Guard), die aufgeklappt die gerenderte Beschreibung des Problems (`renderSolutionMarkdown`, folgt demselben globalen EN/DE-Display-Locale wie die Lösung) und Status/AI/Tag/Regionen-Chips über die geteilte `<ProblemMetaChips>`-Komponente (Single Source of Truth mit dem Problem-Panel, sodass beide Ansichten nicht mehr divergieren) zeigt, und der Vote-Score erscheint als beschriftete „▲ N Stimmen"-Pill (vue-i18n-Plural) statt des nackten „↑ N" in der Top-Leiste, die jetzt „Lösungsansatz i von n" zeigt; der Band-Hintergrund nutzt `rgb(var(--th-surface) / 0.55)` und tönt sich damit pro Theme statt hardcoded rgba; kein Band, kein Toggle nur wenn das Problem weder Beschreibung noch Tags/Regionen hat), dazu einheitliches Modal-Chrome über Problem- und Lösungs-Modal (T-15 — ein geteilter Frosted-Backdrop (`rgb(8 5 14 / 0.28)` + `blur(30px)` dark / `rgb(24 16 32 / 0.20)` light, auf `isDark` gekeyt, damit auch Non-Glass-Themes ihn bekommen) lässt die Mesh-Farben weich durchschimmern statt zu hartem Schwarz abzudunkeln, plus identische Shell-Geometrie (40px Overlay-Padding, 24/28 Header-Top/Body, 16/28 Sticky-Footer) fluchtet beide Modals oben und unten — die R-02 Modal-Surface selbst zunächst unangetastet (nur Border + Schatten später verstärkt, siehe unten); der getönte Scrim ist die bewusste R-09-Ausnahme, theme-unabhängiges `rgb(…/α)` erlaubt; ein Follow-up gab dem Split-Editor-Lösungs-Modal eine feste Fill-Höhe (`min(720px, 100vh − 80)`), sodass es tatsächlich mit dem Problem-Modal fluchtet (~40px Rand) statt auf Content-Höhe zu schweben, und stellte den Problem-Submit-Button vom Brand-Gradient auf das solide `--th-accent` um, das jeder andere CTA nutzt; und da der kaum abgedunkelte Frosted-Backdrop die Modal-Kante mit dem Mesh verschwimmen ließ, bekamen beide Form-Modals einen sichtbaren 1px-Border (dark `rgba(255,255,255,0.15)`, light `rgba(0,0,0,0.10)`) plus einen tieferen zweilagigen Drop-Shadow, sodass die Surface als klar abgehobene schwebende Karte liest — Fill-Farbe und Blur unverändert, die zwei Form-Modals divergieren also bewusst nur bei Border/Schatten von der kanonischen R-02-Surface), und die „+ Problem erfassen"-Aktion in der TopBar ist auf Routen deaktiviert, auf denen das Problem-Formular nicht gemountet ist (Lösungs-Vollansicht, Settings, Status, Admin) — ein Klick dort setzte zuvor nur das Modal-Flag, ohne dass etwas rendern konnte, sodass das Modal beim nächsten Zurückwechseln zur Karte/Tabelle als Ghost aufpoppte; dasselbe Routen-Gate (ein einziges `isProblemListView`-Computed für `/` und `/table`) deaktiviert jetzt auch das Such-Feld und den KI-Suche-Toggle auf diesen Routen, die nur den Graph/die Tabelle filtern und dort sonst bedienbar-aber-wirkungslos waren (⌘K ist dort ebenfalls no-op), dazu eine vereinheitlichte Eck-Radius-Skala (T-17 — Radien kollabieren von ~10 Ad-hoc-Werten auf drei: 8px für Struktur-Container/Controls/Chips, 12px für Cards/Modals, `full` für Pills/Kreise; Tabellen-Header und Panel-Karte gingen beide 5px→8px, um ihren gemeinsamen Radius zu erhalten, Cytoscape-Canvas-Nodes ausgenommen), und das geteilte Modal-Chrome wurde in eine echte `AppModal.vue`-Shell ausgelagert (T-18 — das `Teleport to="body"`, der Frosted-Backdrop, die Glass-R-02-Card-Surface, ESC- und Backdrop-Klick-schließen, die beide Form-Modals dupliziert hatten, liegen jetzt in einer Komponente; Caller übergeben `open` + einen `cardStyle`-Geometrie-Override und füllen einen Default-Slot, sodass SolutionForm seine feste Fill-Höhe und ProblemForm seine `max-height` behält — reiner DRY-Refactor mit byte-identischem Chrome, keine sichtbare Änderung), und das Problem-Kontext-Band-Chrome (T-14) — die klick-/tastatur-bedienbare Headline-Hülle, die gerenderte Markdown-Beschreibung, der aufgeklappte Detail-Body (Beschreibung + Status-/AI-/Tag-/Regionen-Chips + Autor) und die beschriftete Vote-Pill — wurde in geteilte `ProblemContextBand`/`ProblemDescription`/`ProblemContextDetail`/`ProblemVotePill`-Komponenten ausgelagert (plus ein `useProblemMeta`-Composable, das die Tag-/Regionen-Chips genauso auflöst wie das Panel), die jetzt sowohl die Vollansicht als auch das New-Solution-Modal nutzen, wo das Problem beim Öffnen des Modals eager geladen wird (damit die Vote-Pill schon in der eingeklappten Headline steht) und ein Klick auf die „On: <Titel>"-Headline dasselbe volle Detail wie die Full-Page aufklappt — Beschreibung, Meta-Chips und Autor lesen sich in beiden identisch (einmal gefetcht, gleicher Lokalisierungs-Pfad) (`branch: feature/bold-redesign`, 213 Tests grün, merge-ready)
