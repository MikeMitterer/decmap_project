...
**Nachtrag (T-18, 2026-07-27):** Das „**keine** shared Component"-Verdikt gilt weiter fuer die R-02/R-03-**Inline-Werte** an sich — aber der Modal-**Rahmen** der beiden Form-Modals (`ProblemForm`/`SolutionForm`) wurde in eine geteilte `AppModal.vue`-Shell extrahiert, die die R-02-Surface- (inkl. der T-15-Border/Shadow-Elevation) und Frosted-Backdrop-Werte nun **einmal** haelt statt zweimal inline. Die Nicht-Form-Konsumenten (`SolutionPopup`, `ProblemPanel`) tragen die R-02-Werte weiterhin inline — sie sind keine AppModal-Caller. Details: Fund 71.
...
Similarity-Card wurde von `<Teleport to="#panel-status-target">` (ausserhalb der Form) auf inline-render zwischen Title und Description umgestellt — der Status-Slot war ein Side-Panel-Artefakt und nach T-12 kurzzeitig orphan (`#panel-status-target` in `layouts/default.vue`); seit T-16 rendert der Panel-Sticky-Footer (Vote-Pill + Lösungs-Zähler + CTA) hinein — kein Cleanup-Kandidat mehr. Form-Logik (createProblem, validateForm, useSimilarity, useEnglishTranslation, Tag-Erstellung, duplicate_confirmed-Signal) byte-identisch — UI-only-Regel aus dem T-12-Ticket strikt eingehalten. Spec: `apps/frontend/_tickets/solved/T-12-problem-form-modal.md`.
...
**Konkreter Fund 60:** Graph→Solution-Full-Page→Graph-Round-Trip — der ganze View (Selektion + Drill-Level + Zoom/Pan) muss aktiv persistiert werden, sonst ist er nach „Zurück" weg; eigene Fits dürfen dabei nicht als User-Pan zählen (2026-07-23, FE `components/ProblemGraph.vue`, `layouts/default.vue`, `pages/index.vue`, `components/SolutionFullPage.vue`).
Die `/`-Seite hat **kein** `<KeepAlive>` — die Navigation zur Lösungs-Vollansicht unmounted `ProblemGraph.vue` komplett (Cytoscape `destroy()`), Back remountet frisch gegen eine anfangs leere Problems-Liste. Drei Zustände überleben nur, wenn aktiv gespiegelt: **(1) Selektion** → `?problem=<id>` (das Layout persistiert über den Round-Trip, also `watch(() => route.query.problem)` in `default.vue`, sonst liest `index.vue` beim Remount einen stale Wert); **(2) Drill-Level** → neues `?cluster=<tagId>`, restauriert via `restoreDrillView(leaf)`, das die Ahnen-Kette aus dem Leaf baut — **nicht** `navigateToProblem`, das zum tiefsten Problem-Tag über-drillt; **(3) Drill-Viewport** → `graph-viewport-drill` pro Scope, beim `onUnmounted` gespeichert (= exakter Verlasse-Zustand), breiten-robust via Modell-Mittelpunkt (Detail-Panel offen/zu ändert die Graph-Breite). Beide Query-Params reisen im `?from=` der Back-Navigation mit; die Rail-Links der Full-Page reichen denselben `?from=` weiter (`railTarget`), während der Share-`solutionUrl` origin-frei bleibt (gleicher Open-Redirect-Guard wie `backTarget`). **Programmatik-Guard:** eigene Fits/Center stempeln `markProgrammaticView()` (600ms-Fenster deckt die Layout-Animation); der geteilte `viewport`-Listener ignoriert Events innerhalb des Fensters (`isProgrammaticViewActive()`), sonst zählte der eigene Restore-Fit als User-Pan und überschriebe den gespeicherten Viewport oder gäbe nie an den User ab — die erste echte User-Pan/-Zoom setzt `restoreDrillViewport = null`. Ergänzend baut `debouncedRerender` die lokalisierten Node-Labels vor dem Render neu auf (`updateLocalizedLabels`, nur `locale != 'en'`), weil `initGraph` beim Remount gegen die noch leere Liste lief → sonst zeigten die Knoten den englischen Canonical-`title`. Tests: `SolutionFullPage.spec.ts` deckt die `?from=`-Weitergabe im Rail + Open-Redirect-Guard ab (184 FE grün). Regel: ohne `<KeepAlive>` ist jeder View-Zustand, den der User nicht in URL/Storage sieht, nach einem Route-Wechsel verloren — persistieren, was wiederhergestellt werden soll, und im geteilten Viewport-Listener eigene programmatische Kamera-Ops von echten User-Gesten trennen (Zeitfenster-Guard), statt beide gleich zu behandeln.

**Konkreter Fund 61:** Zwei Ansichten desselben Konzepts driften auseinander — die Meta-Chips eines Problems im Problem-Panel und im Solution-Full-Page-Band waren visuell verschieden (Panel: farbcodierter Status + Kategorie-/User-Tag-Unterscheidung + Regionen; Band: nur simple Tag-Chips). Fix nicht durch Markup-Kopie (divergiert erneut), sondern durch Extraktion einer rein präsentationalen SSoT-Komponente (2026-07-23, Commit `01e33d9`, FE `components/ProblemMetaChips.vue`, `ProblemPanel.vue`, `SolutionFullPage.vue`).
`ProblemMetaChips.vue` nimmt **bereits aufgelöste, lokalisierte** Props (`status`, `isAiGenerated`, `tags` mit `level`, `regions`) — keine Datenschicht-Abhängigkeit, daher trivial mit Fixture-Props testbar (`ProblemMetaChips.spec.ts`, 6 Fälle: Status-Badge, Level-10-`#name` vs. Kategorie-Gradient-Dot, Regionen, AI-Badge, `clickable`-`tag-filter`-Emit vs. inerte Spans). Beide Verwender liefern die Chips aus ihrer eigenen Tag-/Region-Quelle (`ProblemPanel` via `metaTags`/`metaRegions`, `SolutionFullPage` via `useTags`/`useRegionsFetch`), die Komponente rendert nur — so bleibt die Präsentation garantiert identisch, während jede Seite ihre Daten selbst auflöst. Panel-Umstellung verhaltens-erhaltend (`tag-filter`-Emit erhalten), verifiziert über die volle Suite (Vitest 192/192), da der Panel keinen eigenen Spec hat. Regel: divergierende Darstellungen desselben Konzepts konsolidiert man in einer präsentationalen Komponente mit vorab aufgelösten Props (Datenauflösung bleibt beim Aufrufer) — nicht durch dupliziertes Markup, das erneut auseinanderläuft.

**Konkreter Fund 62:** Ein offenes Problem-Panel verlor seinen Inhalt, sobald „+ Problem erfassen" das Add-Modal öffnete (und blieb leer nach dem Schließen) — Legacy aus der Prä-T-12-Zeit, als das Formular *im* Panel lag und die Selektion daher weichen musste (2026-07-23, Commit `54cdb48`, FE `layouts/default.vue`, `pages/index.vue`, `pages/table.vue`).
Das Layout inkrementierte beim Form-Öffnen ein `showFormSignal`, worauf `index.vue`/`table.vue` per `watch` `selectedProblem = null` setzten. Seit T-12 ist das Formular ein eigenes zentriertes Modal (`<Teleport to="body">`), das Panel soll hinter dem Backdrop unberührt bleiben — der Selektions-Clearer war toter, schädlicher Rest der Migration (zweites Residuum nach dem `pages/table.vue`-Mount-Pfad aus dem T-12-Migrations-Gotcha in `CLAUDE.md`). Fix: beide `watch(showFormSignal)`-Clearer entfernt; `showFormSignal` war damit unbenutzt → komplett ausgebaut (Layout-Ref/Increment/Provide, beide Injects), in `table.vue` fiel das nur dafür existierende `switchingToForm`-Flag weg (der `closePanel`-Guard vereinfacht sich zu `if (!problem) closePanel()`). Kein Spec angepasst: die Seiten hängen an keinem Unit-Test (nur ein e2e-Spec außerhalb der Unit-Suite), verifiziert über die volle Suite (Vitest 194/194, eslint 0 Errors) + den eindeutigen Code-Pfad (der einzige Ort, der `selectedProblem` beim Form-Öffnen nullte, ist weg). Regel: nach einem UI-Pattern-Wechsel (Panel-Form → Modal) auch die *begleitenden* State-Watcher auditieren, die den alten Kontext voraussetzten — nicht nur die Render-/Mount-Pfade (vgl. T-12-Migrations-Gotcha); ein Watcher, der einen Zustand für einen inzwischen entfernten Grund zurücksetzt, ist stiller Datenverlust.

**Konkreter Fund 63:** Modal-Kontrast/-Konsistenz getunt — beide Form-Modals (Problem/Solution) bekommen einen „Frosted"-Backdrop + einheitliche Shell-Geometrie, aber die R-02 Modal-Surface selbst bleibt unangetastet (Mike-Vorgabe: Farbänderung am Modal zu weitreichend); zugleich wird die T-14-Theming-Entscheidung als kanonische Regel **R-09** promoted (2026-07-23, Commit `1f8c403`, T-15, FE `components/ProblemForm.vue`, `components/SolutionForm.vue`).
Backdrop „Frosted" (Variante A): `rgb(8 5 14 / 0.28)` + `blur(30px)` dark / `rgb(24 16 32 / 0.20)` + `blur(30px)` light — kaum abgedunkelt, starker Blur, die Mesh-Farben schimmern weich durch statt zu hartem Schwarz; auf `isDark` gekeyt, damit auch Non-Glass-Themes den Look bekommen (ersetzt den früheren dunkleren `rgba(0,0,0,0.55)`+`blur(8px)`-Scrim). Einheitliche Shell-Geometrie beider Modals: Overlay-Padding `40`, Header `24/28/0`, Body `24/28`, Sticky-Footer `16/28` — Solution vorher Overlay `32`, Body `py-5`, Footer `14`, jetzt identisch zum Problem-Modal; Header-Top (`24`) und Footer-Bottom (`16`) fluchten bei beiden. **R-09 (Surface-/Tint-Token-Regel):** Der in T-14 gegebene Hinweis „Tint-/Surface-Flächen immer `rgb(var(--th-*) / α)`, nie hardcoded Light/Dark-`rgba`-Paare" (vgl. Gotcha „Komponenten-Inline-rgba bricht Theme-Switch" in `CLAUDE.md`) ist als Design-System-Reverse-Ticket **R-09** kanonisiert. **Bewusste R-09-Ausnahme:** der Modal-Scrim darf ein theme-**un**abhängiges getöntes `rgb(… / α)` sein (der Scrim soll gerade *nicht* pro Theme mitfärben) — kein Token-Verstoß. **Finding (nicht erzwungen):** Problem- und Solution-Modal tragen weiterhin **zwei getrennte** Wrapper mit byte-identischem Backdrop + eigener Teleport/ESC-Logik; ein Merge zu einer geteilten `<AppModal>`-Shell wäre der saubere Anti-Divergenz-Weg, aber >40 Zeilen und ESC/Teleport-betroffen (echtes Verhalten, kein reiner Präsentations-Extract wie `ProblemMetaChips`) → als **Reverse-Ticket R-10** an Design gemeldet und im UI-only-Ticket nicht erzwungen (die beiden Backdrops bis dahin bewusst wertgleich gehalten). **Mike-Go 2026-07-27:** R-10 (+ das gefaltete R-02 Modal-Chrome) ist jetzt als eigenes Ticket **`T-18` (AppModal-Shell)** angelegt (`ready`, „bald umsetzen, DRY") — die geteilte `<AppModal>`-Shell (Teleport/Backdrop/ESC/Chrome/Geometrie) wird aus `ProblemForm`+`SolutionForm` extrahiert, die R-02-Chrome-Werte reisen als DS-Handoff aus T-18. Reiner Chrome-Change (`git revert`-bar), kein neuer Spec (Specs prüfen keine Backdrop-/Padding-Werte), Vitest 194/194, eslint 0 Errors. Regel: Backdrop/Scrim ist die eine bewusste Ausnahme der Surface-Token-Regel R-09; und einen UI-only-Ticket-Scope nicht mit einer >40-Zeilen-Konsolidierung sprengen — Divergenz als Finding melden und die Duplikate bis dahin wertgleich halten.

**Konkreter Fund 64:** Identische Shell-Geometrie fluchtet zwei Modals **nicht**, wenn eines content-höhen-getrieben ist — der Live-Vergleich nach Fund 63 zeigte das Solution-Modal mittig schwebend (~160px Rand oben/unten) neben dem satt sitzenden Problem-Modal (~44px), trotz byte-gleicher Paddings (2026-07-24, Commit `bcd4794`, T-15-Follow-up, FE `components/SolutionForm.vue`, `components/ProblemForm.vue`).
Ursache: das Solution-Modal war `max-height: min(840px, 100vh-64)` — bei kurzem Split-Editor-Content bleibt es unter der Max-Höhe und zentriert sich mit viel Luft, das Problem-Modal (längeres Formular) füllt seine Max-Höhe fast aus und sitzt satt. Gleiche Paddings ≠ gleiche Außenwirkung, solange die Höhen-**Strategie** divergiert. Fix: feste **Fill-Höhe `height: min(720px, calc(100vh - 80px))`** — das Modal füllt jetzt konsistent (~40px Rand, `-80` = 2× das 40px-Overlay-Padding), der Split-Editor bekommt dabei mehr Schreibfläche. Zweiter, gemeinsam gefundener Punkt: der Problem-Submit-Button nutzte als einziger CTA noch den Brand-Gradient `var(--dm-grad)` (Violett-Ende `#8B47C9`) und wirkte „dumpfer" — auf das solide `rgb(var(--th-accent))` + accent-getönten Schatten umgestellt, angeglichen an SolutionForm-Submit/TopBar/Panel/Full-Page (Brand-Gradient bleibt Identitäts-Flächen vorbehalten, siehe README-Regel). Kein Spec (Specs prüfen keine Höhen-/Farbwerte), Vitest 194/194, eslint 0 Errors. Regel: zwei Layouts fluchten nur, wenn sie **dieselbe** Größen-Strategie teilen (fixe Fill-Höhe vs. content-getriebene Max-Höhe) — Paddings anzugleichen reicht nicht; und ein am Live-Stand nachgemessener Abstand schlägt einen Code-Token-Vergleich (der Fund-63-„fluchtet oben und unten"-Claim war ein Over-Claim, erst die Live-Messung deckte die Divergenz auf).

**Konkreter Fund 65:** Ein barely-dimmed Frosted-Backdrop braucht eine abgehobene Modal-Kante — sonst verschwimmt die Surface mit dem Hintergrund; der Fund-63-Vorbehalt „R-02-Surface unangetastet" wird für die Border/Schatten-Achse bewusst aufgehoben (2026-07-24, Commit `ba7ac34` + Follow-up, FE `components/ProblemForm.vue`, `components/SolutionForm.vue`).
Nach Fund 63 (Frosted-Backdrop, kaum abgedunkelt) blieb die Modal-Kante auf dem durchschimmernden Mesh unsichtbar — die kanonische R-02-Inset-Hairline (`0 0 0 1px rgba(255,255,255,0.04) inset`) hob zu wenig ab. Beide Form-Modals bekamen daher einen **sichtbaren 1px-Border** (glass-dark `rgba(255,255,255,0.15)`, glass-light `rgba(0,0,0,0.10)`) statt der Inset-Hairline plus einen **tieferen zweilagigen Drop-Shadow** (`0 40px 120px -24px … , 0 16px 48px -16px …`); **Fill-Farbe + Blur bleiben unverändert**. Damit divergieren `ProblemForm`/`SolutionForm` (`formContainerStyle`) bewusst von der kanonischen R-02-Surface — anders als der ausdrückliche Fund-63-Vorbehalt „R-02-Surface unangetastet", der nur für die ursprüngliche T-15-Chrome-Runde galt. Reiner Chrome-Change (`git revert`-bar), kein Spec (Specs prüfen keine Border-/Schatten-Werte), Vitest 194/194. Regel: eine kanonische Surface-Spec ist kein Dogma — wenn eine bewusste Backdrop-Änderung (Fund 63) die ursprüngliche Elevation-Annahme bricht, ist der scoped Bruch der Spec (nur die Form-Modals, nur Border/Schatten, Fill unverändert) der richtige Fix, dokumentiert statt still.

**Konkreter Fund 66:** Ein `+`-Button, der ein Modal öffnet, das auf der aktuellen Route gar nicht gemountet ist, produziert einen „Ghost-Open" — nichts erscheint beim Klick, das Modal poppt erst beim Zurücknavigieren auf eine Form-Route auf; Fix ist den Button dort zu **disablen**, nicht die Form überall zu mounten (2026-07-24, Commit `00762f6`, T-11-Test-Folgefund von Mike, FE `components/DmTopBar.vue`, `layouts/default.vue`).
Die ProblemForm lebt nur in `pages/index.vue` (`/`) und `pages/table.vue` (`/table`). Auf der Solution-Full-Page (`/problem/*/solution/*`, nutzt dasselbe Default-Layout inkl. `DmTopBar`) sowie `/settings`/`/status`/`/admin` setzte der `+`-Klick nur das Layout-`isProblemFormModalOpen=true`, ohne dass eine `<ProblemForm>`-Instanz zum Rendern existierte → der Flag blieb gesetzt und das Modal erschien erst nach dem Zurückwechseln auf `/`/`/table`. Fix (Mikes Wahl: Button disablen statt zur Karte zu navigieren): `DmTopBar` bekommt eine `canCreateProblem`-Prop (`withDefaults`, default `true`) → `:disabled` + Tooltip `topbar.add_problem_unavailable` + Disabled-Styling (`opacity-40`, `cursor-not-allowed`); das Layout berechnet `canCreateProblem = route.path === '/' || route.path === '/table'` und guardet `handleOpenForm` zusätzlich als Backstop (defense-in-depth, falls über einen anderen Pfad aufgerufen). Der unconditional `open-form`-Emit-Vertrag bleibt unangetastet (siehe Gotcha „`+`-Button-Auth-Entscheidung nur im Container" in `CLAUDE.md`) — der Button wird nur deaktiviert, nicht auth- oder route-verzweigt im Button selbst. Neuer Contract-Test `tests/components/DmTopBar.spec.ts` (2 Fälle): default (`canCreateProblem` true) → Button enabled, ein `open-form`-Emit; `canCreateProblem: false` → Button `disabled`, `title` = `topbar.add_problem_unavailable`, Klick emittiert **nichts** (nativer Disabled-Button). Vitest 196/196, eslint 0 Errors. Regel: ein Button, der einen Zustand in einem Container setzt, der auf der aktuellen Route nicht existiert, ist ein stiller No-op mit Spätfolge — den Button an der Erreichbarkeit seines Ziels gaten (disablen), statt das Ziel überall zu mounten oder den Klick still verpuffen zu lassen.
**Follow-up (2026-07-24, Commit `f1dc546`):** Dieselbe Klasse traf auch das Such-Feld und den KI-Suche-Toggle in `DmTopBar` — beide filtern nur den Graph/die Tabelle, waren auf der Solution-Full-Page (+ Settings/Status/Admin) aber weiter bedienbar und dort wirkungslos. Statt eine zweite Route-Prüfung zu duplizieren wurde das `canCreateProblem`-Computed im Layout zum neutralen **`isProblemListView`** (`route.path === '/' || route.path === '/table'`) verallgemeinert und gated jetzt **beide** Props (`:can-create-problem` + neue `:search-available`). `DmTopBar` disabled bei `searchAvailable: false` Such-Input **und** KI-Toggle (`:disabled` + Tooltip `topbar.search_unavailable` + Disabled-Styling), und der ⌘K-Shortcut ist no-op. Spec um zwei Fälle erweitert (Such-/Toggle-enabled default; disabled → Tooltip + kein Emit), Vitest 198/198. Regel: ein Route-Sichtbarkeits-Gate ist ein wiederverwendbares Prädikat — beim zweiten Verwender neutral benennen und teilen, nicht pro Aktion neu prüfen.

**Konkreter Fund 67:** Vorab-übersetzte Tag-Namen (`tags.name_translations`) statt Live-`/ai/translate` — Contract-Erkenntnisse an drei Nahtstellen (2026-07-24, BE `a355520` Migration 011, AI `db6f41c`).
Kategorie-/Cluster-Namen (L1–L9) sind KI-generiert und englisch; das DE-Frontend übersetzte sie bei **jedem** Table-/Panel-Load live (`translateForDisplay(t.name)`, ~13 Tags → nach ~7 Requests `/ai/translate`-429, nginx 5r/m). Fix: `name_translations` JSONB auf `tags` (analog `problems.original_translations`), beim Clustering-Upsert mit-übersetzt + Einmal-Backfill. Drei Contract-Tests deckten je eine nicht-offensichtliche Invariante ab: **(1) ON-CONFLICT-Refresh statt Ignore** — ein Re-Upsert desselben `(name, level)` muss die gespeicherten Übersetzungen **aktualisieren**, nicht die alten behalten (`test_upsert_tag_refreshes_translations_on_conflict`); sonst friert ein einmal falsch übersetztes Label ein. **(2) Backfill-Scope** — der Einmal-Backfill darf **nur strukturelle Tags ohne** Übersetzung anfassen; L10-User-Tags (frei, kein KI-Label) und bereits vollständige Tags werden übersprungen (`test_backfill_tag_translations_only_untranslated_structural_tags`), sonst kostet jeder Lauf unnötige LLM-Calls und überschreibt evtl. Manuelles. **(3) Provider-scoped Model über eine DRY-Factory** — die Übersetzung läuft auf der **eigenen** Provider-Achse (`translation_provider` + `*_translation_model`), nicht dem `llm_provider` von Spam/Clustering; `build_translation_service()` ist die **einzige** Construction-Stelle (vorher 3: Dependency, Scheduler, Reindex/Approve-Hooks — drift-anfällig), `get_translation_service` ist nur der FastAPI-Dependency-Wrapper (`test_get_translation_service_selects_provider_scoped_model` prüft die Modellwahl je Provider). Regel: verlagert ein Feature eine Live-Übersetzung in eine vorab-berechnete Spalte, gehören drei Dinge in die Contract-Tests — Refresh-Semantik beim Re-Write, Idempotenz/Scope des Backfills und eine einzige Factory für die Service-Konstruktion (sonst driften die manuellen Construction-Sites in Provider/Modell auseinander). **Frontend-Rollout (`c9820da`, `feature/bold-redesign`):** `Tag.nameTranslations` (required) + `utils/tagDisplay.ts` (`tagDisplayName(tag, locale)` = EN-Canonical für `en`, sonst `nameTranslations[locale] ?? name` — **außer L10-User-Tags**: der Guard `locale === 'en' || tag.level >= 10` gibt immer den Roh-`name` zurück, weil L10 die eigenen Worte des Autors sind, kein KI-Label), `realTags.mapTag` mappt `name_translations → nameTranslations`. Umgestellt: `table.vue`, `pages/problem/[id].vue`, `ProblemPanel.vue`, `SolutionFullPage.vue` — deren `localizedTagNames`-Live-Translate-Blöcke + `watch(tags)` sind raus, `metaTags`/`getTagLabel` nutzen `tagDisplayName`; `ProblemMetaChips.vue` ist präsentational und bekommt die lokalisierten Namen von diesen Callern. **Kein Live-Translate für Tags mehr:** auch die letzte Stelle — die Cytoscape-Cluster-Node-Labels + der Breadcrumb-Chip in `ProblemGraph.vue` — ist umgestellt (Commit `580dae7`, 2026-07-24); der `localizedTagLabels`-Ref und der Tag-Zweig in `updateLocalizedLabels` sind entfernt (die Node-Labels lesen synchron `tagDisplayName(tag, locale)`, `updateLocalizedLabels` übersetzt nur noch **Problem-Titel**). Der Graph-View war Mikes eigentlicher 429-Live-Auslöser: das Umschalten in die Graph-View feuerte einen `/translate`-Burst über **alle** sichtbaren Cluster-Nodes gleichzeitig. Nur die 4 Tag-Hunks wurden per `git apply --cached` committet — die parallele T-11-Graph-Baustelle im selben File bleibt uncommitted. Frontend-Contract-Erkenntnis: der Unit-Test `tests/utils/tagDisplay.spec.ts` fixiert die Fälle von `tagDisplayName` (EN-Canonical / DE-Übersetzung / Fallback auf `name` bei fehlender Übersetzung / leere `nameTranslations` / **L10-User-Tag nie übersetzt, auch bei vorhandener Übersetzung**). Der L10-Guard ist explizit im Code (`tag.level >= 10 → Roh-name`), nicht bloß eine Daten-Koinzidenz (der Backfill überspringt L10) — die „eigene Worte des Autors nie LLM-rückübersetzen"-Regel steht damit an **einer** erzwungenen Stelle, drift-fest gegen ein künftig versehentlich gesetztes L10-`name_translations`. Vgl. Fund 26 (Rest-429 an reinen Display-Stellen) und Features-Doc, Abschnitt „Tag-Namen-Lokalisierung".

**Konkreter Fund 68:** Der „mike"-User-Tag-Bug — ein User-Tag ließ sich nur **anlegen** („Create"), existierende Tags erschienen nie als Vorschlag; ein zweites Anlegen desselben Labels warf einen irreführenden CORS-Fehler im Browser (2026-07-24, FE `ProblemForm.vue`/`realTags.ts`, BE `routers/tags.py`). **Zwei unabhängige Ursachen.** **(1) Anzeige-Seite:** `ProblemForm.vue` rief nie `fetchTags()` auf; `useTags()` ist **per-Instance-State** (eigene, leere `tags`-Liste je Composable-Aufruf), also blieb die Tag-Liste des Formulars leer → jeder getippte Name sah neu aus, `canCreateTag` war immer `true` → es bot **nur** „Create" an, ein bestehendes Tag wurde nie als auswählbarer Vorschlag gezeigt. Fix: `onMounted(fetchTags)` im Formular. **(2) Persistenz-Seite (Backstop):** `create_tag` (Backend) fügte blind ein → ein Duplikat verletzte den `(name, level)`-Unique-Constraint → **unbehandelter `IntegrityError` → 500**; und weil ein 500 **keine CORS-Header** trägt, erschien der Fehler im Browser als irreführendes „blocked by CORS" (die CORS-Meldung war ein Red Herring, kein CORS-Problem). Fix: `create_tag` ist idempotent — ein existierendes **nicht-gelöschtes** L10-Tag wird **case-insensitiv** (`func.lower(name) == …, level == 10`) wiederverwendet statt dupliziert; der Endpoint gibt dann das bestehende Tag zurück (201). Konsequenz im Frontend: `realTags.createTag` **upsertet** in die lokale Liste (Backend kann ein bestehendes Tag zurückgeben) statt blind zu appenden — sonst landet das wiederverwendete Tag doppelt. **(2b) Security-Nachtrag (Push-Review, `5047279`):** Der erste Idempotenz-Fix belebte bei Namenskollision ein **soft-deleted** Tag wieder — aber Tag-Löschung ist superuser-only (`delete_tag`, Moderations-Aktion), also konnte jeder eingeloggte User eine Moderations-Löschung durch erneutes Posten des Namens aufheben (**Authorization-Bypass**). Behoben: nur nicht-gelöschte Matches werden wiederverwendet; kollidiert der Name **ausschliesslich** mit einem soft-deleted Tag (das den `(name, level)`-Unique-Slot belegt), antwortet der Endpoint mit **`409`** statt zu reviven (und statt am Constraint zu 500en). Drei Backend-Contract-Tests fixieren das Verhalten: Re-POST desselben Labels gibt dieselbe `id` zurück (`test_create_existing_tag_reuses_it`), case-insensitiv (`test_create_tag_dedup_is_case_insensitive`), und ein moderativ entferntes (soft-deleted) Tag lässt sich **nicht** neu anlegen → 409, bleibt entfernt (`test_recreate_soft_deleted_tag_rejected_not_revived`). Backend 11 Tag-Tests grün, ruff clean; Frontend 204 grün. **Drei Regeln:** per-Instance-Composable-State muss **dort geladen** werden, wo er gelesen wird (ein zweiter `useTags()`-Consumer teilt nicht die Liste des ersten); ein unbehandelter DB-Constraint-Fehler wird zum 500, ein 500 ohne CORS-Header **maskiert sich als CORS-Fehler** — bei „CORS"-Meldungen auf mutierenden Endpoints zuerst nach einem serverseitigen 500 suchen, nicht der CORS-Config nachjagen; und ein „idempotentes Create" darf **nie** eine per Moderation/Superuser gelöschte Ressource reviven (sonst umgeht ein normaler User das Auth-Gate der Löschung).

**Konkreter Fund 69:** Ein gebatchter Page-Resolve deckt nur die Listen-Pfade — die Single-Item-Endpoints tragen dasselbe abgeleitete Feld nur, wenn sie es explizit auflösen (2026-07-25, BE `d8410fd`).
Fund 44/47 lösten `company` + `author_display_name` gebatcht via `_load_authors` auf und reichten sie über `_to_read(company=…, author_display_name=…)` an jedes `ProblemRead` — aber **nur** in `list_problems` + `_list_problems_semantic`. Der Detail-Endpoint `get_problem` (`GET /problems/{id}`) und die create/update/delete-Responses bauten ihr `ProblemRead` über den **nackten** `_to_read(p, tags, regions)` ohne diese kwargs, lieferten beide Felder also als `null`. Sichtbare Folge: das T-14 Problem-Kontext-Band der Solution-Vollansicht (lädt via `fetchProblemById` → Detail-Endpoint) zeigte seine Autor/Firma-Row (`v-if="problem.authorDisplayName || problem.company"`) **nie**, obwohl dieselben Probleme in der Table die Felder tragen — ein Live-Fund im T-14-Browser-Check, nicht aus der Route-Signatur ablesbar. Fix (DRY): der neue Helper `_read_with_author(session, p, tags, regions)` löst das Autor-Profil wie die Liste auf und ist die **einzige** Konstruktionsstelle für Single-Problem-`ProblemRead`s (Detail-GET + create/update/delete) → alle tragen die Felder konsistent, kein pro-Endpoint-Auslassen mehr. Regressionstest `test_detail_endpoint_includes_author_profile` prüft, dass der Detail-Response die Felder enthält **und** paritätisch zur Liste ist (Backend-Unit-Suite 198 grün, ruff clean; im selben Zug 20 vorbestehende UP/E-Verstöße in `problems.py` mitbereinigt, separater `style`-Commit `c56d6eb`). Regel: ein „liegt auf jedem `ProblemRead`"-Vertrag (Fund 45/47) gilt erst, wenn **jeder** Konstruktionspfad das Feld auflöst — ein gebatchter Page-Resolve deckt die Listen, aber die Single-Item-/Mutations-Responses brauchen dieselbe Auflösung (idealerweise über **einen** geteilten Helper), sonst hält der Vertrag still nur für den Listen-Pfad.

**Konkreter Fund 70:** Eck-Radien laufen über **eine zentrale 3-Werte-Skala**, nicht über Ad-hoc-Werte — die App hatte ~10 verschiedene Radien (`rounded-lg/xl/2xl/3xl/md`, `rounded-[5/10/20px]`, inline `border-radius` 5/8/9/10/12/14/16px) (2026-07-26, T-17, FE `tailwind.config.ts` + Sweep über components/pages/layouts, Commit `cd16526`).
Die `borderRadius`-Config kollabiert Tailwinds Default-Radien auf **8px** (`sm`/`DEFAULT`/`md`/`lg` → Struktur-Container: Panel, Tabelle, Tabellen-Header, Inputs, Chips, Badges, kleine Controls) und **12px** (`xl`/`2xl`/`3xl` → Inner-Cards, Modals, größere Buttons, Solution-Cards, Footer-CTA); `full` (`9999px`, Pills/Avatare/Toggles) und `border-radius:50%` (echte Kreise) bleiben unberührt. Die Ausreißer wurden manuell auf die Skala gezogen (`rounded-[10px]`/`[5px]` → 8, `rounded-[20px]` → 12; inline 16/14 → 12, 10/9 → 8; das Tabellen-Header-`<style>` von 5px → 8px, womit die T-16-Gleichheit **Panel == Tabellen-Header** erhalten bleibt, jetzt bei 8px). **Regel:** neue UI nutzt ausschließlich die Skala (8 / 12 / `full`) — keine `rounded-[Npx]`-Arbitraries, keine krummen Inline-Radien; ein bewusster Sonderfall wird mit Kommentar als Ausnahme markiert, nicht still gelassen. **Gotcha:** der Config-Remap rendert erst nach einem **Frontend-Dev-Restart** (Tailwind regeneriert die Utility-CSS aus `tailwind.config.ts`; Hot-Reload greift nicht) — direkte Klassen-/Inline-Edits sind dagegen sofort live. Cytoscape-Node-Radien (Canvas, kein CSS-`border-radius`) sind **nicht** Teil des Remaps. ESLint 0 errors, 205/205 Vitest.

**Konkreter Fund 71:** Zwei byte-identische Modal-Wrapper → **eine** `AppModal.vue`-Shell — reiner DRY-Extract, Chrome unveraendert (2026-07-27, T-18, FE `825589a`, setzt das aus T-15 gemeldete Reverse-Ticket R-10 um).
`ProblemForm.vue` und `SolutionForm.vue` duplizierten jeweils den **kompletten** Modal-Rahmen: `Teleport to="body"`, Frosted-Backdrop, Glass-R-02-Card-Surface (inkl. der T-15-Border/Shadow-Elevation, die die zwei Form-Modals bewusst von der kanonischen R-02-Surface divergieren laesst, s. Fund 23), ESC-schließen, Backdrop-Klick-schließen, `@click.stop`. Jede Rahmen-Politur (zuletzt T-15) musste in **beide** Files gepflegt werden. Extrahiert nach `components/AppModal.vue`: Kontrakt = Prop `open` (default `true`, Caller mounten weiter per `v-if`) + `cardStyle` (Geometrie-Override, auf die Card-Surface gemergt), Emit `close`, Default-Slot (Caller liefern ihren eigenen Sticky-Header / scrollbaren Body / Sticky-Footer). Die **geometrie-abweichenden** Werte reisen als `cardStyle`-Prop, nicht hart verdrahtet: SolutionForms feste Fill-Hoehe (`min(720px, 100vh − 80)`, T-15-Follow-up) und ProblemForms `max-height` bleiben so je Caller erhalten. **Regel:** eine geteilte Shell darf nur ihr **eigenes** Chrome hart verdrahten (Backdrop, Z-Index, R-02-Surface, ESC/Teleport) — alles, was pro Caller variiert (Card-Geometrie), gehoert hinter eine Prop, sonst zwingt der Extract einen Caller in die Maße des anderen. Die Chrome-Werte wurden **byte-identisch** aus den Pre-Extraction-Forms uebernommen — kein Redesign; der Diff ist rein struktureller Umbau (`git revert`-bar), Live-Render der beiden Modals unveraendert.
**Contract-Test-Erkenntnis:** Der ESC-`document`-Listener in `AppModal` laeuft **ohne** `import.meta.client`-Guard — `onMounted`/`onUnmounted` sind projekt-konventionell client-only (feuern nie im SSR), also braucht der Listener keinen Guard, und genau das macht den ESC-Pfad im jsdom-Test ueberhaupt erst pruefbar (ein `import.meta.client`-Guard haette ihn im Test-Environment tot gestellt). Vgl. den `localStorage`-auf-SSR-Routen-Gotcha — dort ist der Guard Pflicht (Layout rendert im SSR), hier waere er kontraproduktiv (Komponente mountet nur client-seitig). Regel: den Secure-/SSR-Guard nach dem **tatsaechlichen** Render-Kontext der Komponente setzen, nicht reflexhaft. Neue `tests/components/AppModal.spec.ts` (Slot-Render, ESC-schließen, Backdrop-Klick, `open=false` = kein Render, Unmount-Cleanup des Listeners); `SolutionForm.spec` mountet jetzt **durch** AppModal. Vitest 210/210, eslint 0 Errors. Spec: `apps/frontend/_tickets/T-18-appmodal-shell.md`.

**Konkreter Fund 72:** Das „Problem-Headline klicken → Detail aufklappen"-Muster existierte doppelt (T-14 Full-Page-Band + neu ergaenztes New-Solution-Modal) → **zwei** geteilte Praesentations-Komponenten statt paralleler Reimplementierung — gleiche SSoT-Extraktions-Familie wie Fund 61 (`ProblemMetaChips`) und Fund 71 (`AppModal`) (2026-07-27, T-11-Folgearbeit, FE Commits `e22984a`/`83140ee`).
Zuerst bekam das New-Solution-Modal (`SolutionForm.vue`) dasselbe Aufklapp-Verhalten wie die Solution-Full-Page: ein Klick auf die „On: <Titel>"-Headline (▲/▼-Chevron, Keyboard Enter/Space, `aria-expanded`) klappt die volle Problem-Beschreibung auf; das Problem wird beim ersten Aufklappen **lazy** via `fetchProblemById` geladen (die id liegt vor), ueber denselben Display-Locale-Pfad lokalisiert wie die Full-Page, nur **einmal** gefetcht (Cache), Fehler fail-safe (`consola.warn`, kein Crash). Statt zwei paralleler Implementierungen wurde das Muster in **eine** Quelle gezogen — bewusst **zwei** Komponenten entlang der Verantwortungs-Grenze: **`ProblemContextBand.vue`** kapselt nur die *Interaktion/a11y* (klickbare Headline-Huelle mit `role`/`tabindex`/`aria-expanded` + Enter/Space, ▲/▼-Toggle, aufklappbare Region; `v-model:open` + `expandable`-Prop; Slots `headline`/`trailing`/default fuer die je Aufrufer abweichende Chrome — Vote-Pill, Meta, Autor), **`ProblemDescription.vue`** nur das *Rendering* (lokalisierte Beschreibung als sanitized Markdown via `renderSolutionMarkdown` + der lange `[&_h2]…`-Prose-Klassenblock, vorher in zwei Files dupliziert, jetzt an einer Stelle). `SolutionFullPage` **und** `SolutionForm` konsumieren beide — das Problem liest sich in beiden Ansichten identisch; Full-Page-Verhalten unveraendert (alle `problem-band-*`-Testids erhalten), UI-only (Validation, Voting-Modell, Backend-Endpoints unberuehrt). Neue i18n-Keys `solution.problemLoading`/`solution.noProblemDescription` (en+de); Chevron-Tooltips reusen `solution.showFullProblem`/`showLess`. **Contract-Test-Erkenntnis (identisch zu Fund 71):** `SolutionFullPage.spec` und `SolutionForm.spec` muessen die geteilten Komponenten explizit registrieren (`global.components`), weil der Nuxt-Auto-Import in vitest **nicht** greift — sobald ein Komponenten-Extract von mehreren Specs konsumiert wird, ist die Registrierung im Test-Setup Pflicht, sonst rendert der Slot leer ohne Fehler. Tests: +2 SolutionForm-Specs (lazy expand rendert die Beschreibung; Fetch nur einmal), Vitest 212/212, eslint 0 Errors, beide Flaechen (Modal + Full-Page, Expand/Collapse) live verifiziert. Regel (wie Fund 61/71): dasselbe UI-Muster an zwei Stellen konsolidiert man entlang seiner Verantwortungs-Grenzen in praesentationale Komponenten mit vorab aufgeloesten Props/Slots — Interaktions-Shell und Prose-Renderer getrennt, nicht als ein Monolith — statt Markup zu duplizieren, das erneut auseinanderlaeuft.
