...
- **`EnglishTranslationSection` `isCollapsed` und async Props:** `showFields` transitiert false→true waehrend `loadLocalizedEditFields` laeuft (async) → `watch(showFields)` feuert und expandiert trotz `startCollapsed=true`. Fix: `startCollapsed`-Prop + `onMounted`-Guard (`if (props.showFields && props.startCollapsed) isCollapsed.value = true`). Ohne Komponenten-Test nicht erkennbar — Vitest-Specs fuer Collapse-Verhalten anlegen (Referenz: `EnglishTranslationSection.spec.ts`).
