...
routeRules: {
  ...
  '/problem/*/solution/**': { ssr: false, prerender: false }, // Loesungs-Vollansicht — Client-only (lädt Solutions zur Laufzeit, keine SEO-Fläche); MUSS vor '/problem/**' stehen
...
- **nginx `proxy_pass` mit Variable + `rewrite` — vier Gotchas:** (1) `proxy_pass http://$var/` macht keine Prefix-Substitution — `/ai/health` landet als `/ai/health` beim AI-Service. (2) `rewrite ... break` stoppt auch `set` — `set $var` immer **vor** `rewrite` stellen, sonst bleibt Variable leer → "no host in upstream". (3) `proxy_pass http://$var` ohne URI nach `rewrite` nimmt die Original-URI — `$uri` explizit übergeben: `proxy_pass http://$upstream$uri$is_args$args`. (4) **Variable im `proxy_pass` erzwingt einen `resolver`:** sobald der Upstream eine Variable ist (`proxy_pass http://$upstream_ai…`), löst nginx den Hostnamen **pro Request** auf und ignoriert `/etc/hosts` — ohne `resolver`-Direktive: `[error] no resolver defined to resolve <host>` → **502** (nur für die variablen Locations; statische `proxy_pass http://host:port` funktionieren weiter, da beim Start aufgelöst). Fix: `resolver 127.0.0.11 valid=30s;` (Docker embedded DNS) im `http`-Block. **Prod-`nginx.conf` hatte ihn, Dev-`nginx.dev.conf` fehlte er** (2026-07-22 nachgezogen) — die beiden Configs bei `/ai/`-Routing-Änderungen synchron halten. **Bind-Mount-Nachtrag:** Der Dev-nginx mountet die Config per Volume; nach einem Edit sieht der Container auf macOS oft noch den **alten/abgeschnittenen Inode** (atomic save) → `nginx -t` meldet mitten in der Datei "unexpected end of file". `docker restart decisionmap-nginx-dev` erneuert den Mount; `nginx -s reload` allein reicht nicht.
